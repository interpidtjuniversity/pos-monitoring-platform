...
const PositionsPage = () => {
  ...
  const handleReportClick = async (positionName, positionId) => {
    ...
    try {
      const response = await fetch(`http://host.docker.internal:5000/job_malpractice?name=${encodeURIComponent(positionName)}`, {
        method: 'GET',
        headers: {
  ...
  const handleAddMalpractice = async () => {
    ...
    try {
      const response = await fetch('http://host.docker.internal:5000/add_job_malpractice', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleDeleteMalpractice = async (malpractice) => {
    ...
    try {
      const response = await fetch('http://host.docker.internal:5000/remove_job_malpractice', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleBlur = async (positionId, positionName, field) => {
    ...
    try {
      ...
      if (field === 'workload') {
        url = 'http://host.docker.internal:5000/change_workload';
        paramName = 'workload';
      } else if (field === 'deduction_factor') {
        url = 'http://host.docker.internal:5000/change_deduction_factor';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://host.docker.internal:5000/submit_malpractice_record', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
